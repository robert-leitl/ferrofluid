...
void main() {
    ...
    float NdV = dot(N, V);
    float dif = dot(L, N);
    float specular = powFast(max(0.0, dot(R, -V)), 100.);
    ...
    vec3 ambient = texture(u_envMapTexture, equiPos).rgb;
        ...
        // fresnel
        float ft = dot(N, vec3(0., 0., 1.));
        float fresnel = smoothstep(0.95, 1., min(1., pow(1. - ft, 2.)));
        vec3 fc = (fresnel) * vec3(0., 1., 0.6);
    ...
    vec3 color = ambient * 0.8 + fc * 1.5 + specular * 1.5;

    outColor = vec4(color * edgeMask, 1.);
}